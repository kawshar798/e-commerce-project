...
if(isset($_GET['status'])){
    $Child_cat_id = $_GET['childId'];
    if($_GET['status']=="published"){
        $category->publishedChildCatById($Child_cat_id);
    }
    if($_GET['status']=="unpublished"){
        $category->unpublishedChildCatById($Child_cat_id);
    }
    if($_GET['status']=="delete"){
        $category->deleteChildCatById($Child_cat_id);
    }
    ...
    <div class="x_content">
        <div id="datatable_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer"><div class="row"><div class="col-sm-6"><div class="dataTables_length" id="datatable_length"><label>Show <select name="datatable_length" aria-controls="datatable" class="form-control input-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-6"><div id="datatable_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control input-sm" placeholder="" aria-controls="datatable"></label></div></div></div><div class="row"><div class="col-sm-12">
                    <table id="datatable" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="datatable_info">
                        <thead>
                        <tr role="row">
                            <th class="sorting_asc" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending">Child Category No</th>
                            <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" >Category title</th>
                            ...
                            <th class="sorting" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" >Child Category title</th>
                        ...
                        if($child_cat_info){
                            ...
                            while($child_cat_show = mysqli_fetch_assoc($child_cat_info)){?>
                                ...
                                <tr>
                                    <td><?php echo $i++;?></td>
                                    <td><?php echo $child_cat_show['category_title'];?></td>
                                    <td><?php echo $child_cat_show['sb_cat_title'];?></td>
                                    <td><?php echo $child_cat_show['child_cat_title'];?></td>

                                    <td><?php
                                        if($child_cat_show['publication_status']==1){
                                            echo "published";
                                        }else{
                                            echo "unpublished";
                                        }
                                    ...
                                    <td>
                                        ...
                                        if($child_cat_show['publication_status']==1){?>
                                            <a href="?status=unpublished&&childId=<?php echo $child_cat_show['child_cat_id'];?>" class="btn btn-info" title="Unpublished"><i class="fa fa-arrow-up"></i></a>
                                        <?php }else{?>

                                            <a href="?status=published&&childId=<?php echo $child_cat_show['child_cat_id'];?>" class="btn btn-warning" title="Published"><i class="fa fa-arrow-down"></i></a>
                                        <?php }

                                        ?>

                                        <a href="edit_category.php?childId=<?php echo $child_cat_show['child_cat_id'];?>" class="btn btn-success" title="Edit"><i class="fa fa-edit"></i></a>
                                        <a href="?status=delete&&childId=<?php echo $child_cat_show['child_cat_id'];?>" class="btn btn-danger" title="Delete"><i class="fa fa-trash"></i></a>
