...
$cat_info = $category->show_all_category();

if(isset($_GET['status'])){
    $cat_id = $_GET['cat_id'];
    if($_GET['status']=="published"){
        $category->publishedCatByBid($cat_id);
    }
    if($_GET['status']=="unpublished"){
        $category->unPublishedCatByBid($cat_id);
    }
    if($_GET['status']=="delete"){
        $category->deleteCatByBid($cat_id);
    }
    ...
    <div class="x_content">
        <div id="datatable_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer"><div class="row"><div class="col-sm-6"><div class="dataTables_length" id="datatable_length"><label>Show <select name="datatable_length" aria-controls="datatable" class="form-control input-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-6"><div id="datatable_filter" class="dataTables_filter"><label>Search:<input type="search" class="form-control input-sm" placeholder="" aria-controls="datatable"></label></div></div></div><div class="row"><div class="col-sm-12">
                    <table id="datatable" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="datatable_info">
                        <thead>
                        <tr role="row">
                            <th class="sorting_asc" tabindex="0" aria-controls="datatable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending">Category No</th>
                        ...
                        if($cat_info){
                            ...
                            while($cat_show = mysqli_fetch_assoc($cat_info)){?>


                                <tr>
                                    <td><?php echo $i++;?></td>
                                    <td><?php echo $cat_show['category_title'];?></td>
                                    <td><?php
                                        if($cat_show['publication_status']==1){
                                            echo "published";
                                        }else{
                                            echo "unpublished";
                                        }
                                    ...
                                    <td>
                                        ...
                                        if($cat_show['publication_status']==1){?>
                                            <a href="?status=unpublished&&cat_id=<?php echo $cat_show['category_id'];?>" class="btn btn-info" title="Unpublished"><i class="fa fa-arrow-up"></i></a>
                                        <?php }else{?>

                                            <a href="?status=published&&cat_id=<?php echo $cat_show['category_id'];?>" class="btn btn-warning" title="Published"><i class="fa fa-arrow-down"></i></a>
                                       <?php }

                                        ?>

                                        <a href="edit_category.php?cat_id=<?php echo $cat_show['category_id'];?>" class="btn btn-success" title="Edit"><i class="fa fa-edit"></i></a>
                                        <a href="?status=delete&&cat_id=<?php echo $cat_show['category_id'];?>" class="btn btn-danger" title="Delete"><i class="fa fa-trash"></i></a>
